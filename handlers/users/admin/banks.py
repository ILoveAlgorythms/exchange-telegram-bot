...
def admin_banks_list(call):
    ...
    bot.edit_message_text(
        ...
        text=_(user['language_code'], 'admin_page_banks'),
...
def admin_create_payment_accoount(message):
    with bot.retrieve_data(message.from_user.id) as data:
        ...
        success_text = _(user['language_code'], 'admin_data_bank_add_accounts_success')
        success_text += _(user['language_code'], key_string)
...
def delete_payment_account(message):
    ...
    inline_back_to = _(lang, 'inline_back_to')
    key_string = _(lang, 'object_not_found')
    kb = None
    ...
    if account:
        ...
        key_string = _(lang, 'object_deleted').format(f"_{account.get('account')}, {account.get('account_info')}, {account.get('account_limit')}_")
...
@bot.callback_query_handler(is_chat=False, func=lambda call: call.data.startswith(callback_data_admin_bank_accept_delete), role=['admin'])
def admin_delete_country_accept(call):
    ...
    bank = db.get_bank(bank_id, name_id="id")
    accounts = db.get_accounts(bank_id, sql="AND status='active'")

    if accounts:
        bot.answer_callback_query(
            call.id,
            _(user['language_code'], 'exist_bank_account'),
            show_alert=True
        )
        return
...
def admin_create_bank(call):
    ...
    bot.edit_message_text(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        text=_(user['language_code'], 'admin_create_bank_name'),
...
def admin_create_bank_a1(message):
    ...
    with bot.retrieve_data(message.from_user.id) as data:
        try:
            ...
            bot.send_message(
                chat_id=message.from_user.id,
                text=_(user['language_code'], 'admin_create_bank_code')
            )
        except Exception as e:
            print(e)
...
def admin_create_bank_a2(message):
    ...
    with bot.retrieve_data(message.from_user.id) as data:
        ...
        bot.send_message(
            ...
            _(user['language_code'], 'admin_create_bank_info').format(data['bank_name'], data['bank_code']),
            reply_markup=MenuKeyboard.accept_or_decline(
                user,
...
def admin_create_bank(call):
    ...
    with bot.retrieve_data(call.from_user.id) as data:
        ...
        bot.edit_message_text(
            ...
            message_id=call.message.message_id,
            text=_(user['language_code'], 'admin_create_bank_success'),
            reply_markup=MenuKeyboard.back_to(
                user,
