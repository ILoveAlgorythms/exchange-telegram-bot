...
locales['ru'] = {
    ...
    'manager_not_exist': 'не назначен',

    'exist_bank_account': '⚠️ Вы не можете удалить банк с активными счетами. Удалите счета перед удалением банка.',
    ...
    'incorrect_arguments': '⚠️ Указаны некорректные аргументы при вызове _{0}_',
    ...
    'exchange_issuing_quotes_debug': "\n\n# *DEBUG DATA:*\n## *Оригинальный курс обмена {from_name} на {to_name}*: 1 {from_name} = {orig2} {to_name}\n## *Получаемая сумма без спреда:* {orig1} {to_name} \n## *Спред:* {origsp}%",
    ...
    'dispute_not_available': '❌ Время для открытия спора по сделке истекло',
    ...
    'admin_search_home': '🔎 *Поиск сделок и пользоваелей*\n\nПервый фильтр отобразит данные о сделке по ID, второй покажет последние сделки пользователя для идентификации, третий откроет профиль с подробной информацией пользователя.',
    ...
    'admin_edit_limits_cancel_time_deals': '⚠️ Введите новый лимит по времени на оплату сделки (когда выданы реквизиты)\n\n__Пример: 15 (15 минут)',
    ...
    'verification_text': 'Для прохождения верификации нажмите на кнопку "☑️ Пройти верификацию карты" или "☑️ Пройти полную верификацию" ниже.',
    ...
    'inline_admin_search_profile': '👤 Поиск профиля',
    ...
    'inline_admin_pair_verification_account_active': '🟢 Верификация счёта: вкл',
    'inline_admin_pair_verification_account_inactive': '🔴 Верификация: выкл',
    ...
    'inline_params_limits_cancel_time_deal': 'Лимит времени на оплату',
    'inline_admin_stats_bank_accoount': '🎡 Оборот по счетам',
    'inline_card_verification': '☑️ Пройти верификацию карты',
    'inline_full_verification': '☑️ Пройти полную верификацию',
